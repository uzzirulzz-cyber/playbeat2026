...
export async function seedDatabaseIfEmpty() {
  try {
    ...
    for (const prod of INITIAL_PRODUCTS) {
      await ProductModel.findOneAndUpdate({ id: prod.id }, prod as any, { upsert: true, returnDocument: 'after' });
    }
    ...
    for (const cat of INITIAL_CATEGORIES) {
      await CategoryModel.findOneAndUpdate({ id: cat.id }, cat as any, { upsert: true, returnDocument: 'after' });
    }

    for (const coup of INITIAL_COUPONS) {
      await CouponModel.findOneAndUpdate({ code: coup.code }, coup as any, { upsert: true, returnDocument: 'after' });
    }
