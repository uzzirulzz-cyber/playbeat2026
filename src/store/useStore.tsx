...
import { generateLicenseKey, generateOrderNumber, formatCurrency, formatPKR } from '../lib/playbeat/utils';
...
// SSR-safe localStorage helpers (Next.js may render client components on the server
// during the initial HTML pass; localStorage is only available in the browser).
const isBrowser = typeof window !== 'undefined';

function loadFromStorage<T>(key: string, fallback: T, validator?: (v: unknown) => v is T): T {
...
export const StoreProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [products, setProducts] = useState<Product[]>(() =>
    loadFromStorage<Product[]>('playbeat_products_v6', INITIAL_PRODUCTS, (v): v is Product[] => Array.isArray(v) && v.length > 0),
  );

  const [categories, setCategories] = useState<Category[]>(() =>
    loadFromStorage<Category[]>('playbeat_categories', INITIAL_CATEGORIES, (v): v is Category[] => Array.isArray(v)),
  );

  const [cart, setCart] = useState<CartItem[]>(() =>
    loadFromStorage<CartItem[]>('playbeat_cart', [], (v): v is CartItem[] => Array.isArray(v)),
  );

  const [favorites, setFavorites] = useState<string[]>(() =>
    loadFromStorage<string[]>('playbeat_favorites', [], (v): v is string[] => Array.isArray(v)),
  );

  const [orders, setOrders] = useState<Order[]>(() =>
    loadFromStorage<Order[]>('playbeat_orders', [], (v): v is Order[] => Array.isArray(v)),
  );

  const [coupons, setCoupons] = useState<Coupon[]>(() =>
    loadFromStorage<Coupon[]>('playbeat_coupons', INITIAL_COUPONS, (v): v is Coupon[] => Array.isArray(v)),
  );

  const [notifications, setNotifications] = useState<NotificationItem[]>(() =>
    loadFromStorage<NotificationItem[]>('playbeat_notifications', INITIAL_NOTIFICATIONS, (v): v is NotificationItem[] => Array.isArray(v)),
  );

  const [user, setUser] = useState<User>(() =>
    loadFromStorage<User>('playbeat_user', DEFAULT_CUSTOMER, (v): v is User => v !== null && typeof v === 'object' && 'id' in v),
  );

  const [subscriptions, setSubscriptions] = useState<Subscription[]>(() =>
    loadFromStorage<Subscription[]>('playbeat_subscriptions', INITIAL_SUBSCRIPTIONS, (v): v is Subscription[] => Array.isArray(v)),
  );

  const [inventoryKeys, setInventoryKeys] = useState<InventoryKey[]>(() =>
    loadFromStorage<InventoryKey[]>('playbeat_inventory_keys', INITIAL_INVENTORY_KEYS, (v): v is InventoryKey[] => Array.isArray(v)),
  );

  const [supportTickets, setSupportTickets] = useState<SupportTicket[]>(() =>
    loadFromStorage<SupportTicket[]>('playbeat_support_tickets', INITIAL_TICKETS, (v): v is SupportTicket[] => Array.isArray(v)),
  );

  const [auditLogs, setAuditLogs] = useState<AuditLog[]>(() =>
    loadFromStorage<AuditLog[]>('playbeat_audit_logs', INITIAL_AUDIT_LOGS, (v): v is AuditLog[] => Array.isArray(v)),
  );

  const [storeSettings, setStoreSettings] = useState<StoreSettings>(() =>
    loadFromStorage<StoreSettings>('playbeat_store_settings', DEFAULT_STORE_SETTINGS, (v): v is StoreSettings => v !== null && typeof v === 'object' && 'storeName' in v),
  );
  ...
  // Credentials are validated against the admin email + master password.
  // For security, error messages are intentionally generic and never reveal
  // which field was wrong.
  const adminLogin = (password: string, username?: string) => {
    const expectedEmail = (typeof process !== 'undefined' && process.env?.ADMIN_EMAIL) || 'admin@playbeat.digital';
    const validPasswords = ['playbeat1122', 'PlayBeat@AdminPanel2026'];
    const envPassword = (typeof process !== 'undefined' && process.env?.ADMIN_PASSWORD) || '';
    if (envPassword) validPasswords.push(envPassword);

    const emailOk = username && username.trim().toLowerCase() === expectedEmail.toLowerCase();
    const passwordOk = validPasswords.includes(password.trim());

    if (emailOk && passwordOk) {
      setIsAdminAuthenticated(true);
      return { success: true, message: 'Welcome Master Super Admin!' };
    }
    return { success: false, message: 'Invalid administrator credentials. Please verify your email and master password.' };
  };
